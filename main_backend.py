...
# Confidence formula constant: prevents edge cases at the clustering boundary
CONFIDENCE_SAFETY_MARGIN = 1.2

# Cached person centroids — invalidated whenever faces are tagged/untagged
_centroids_cache = None

def _invalidate_centroids():
...
def _get_centroids():
    ...
    if _centroids_cache is None:
        ...
        _centroids_cache = compute_person_centroids(known_faces) if known_faces else {}
...
async def get_faces(status: str = "all", cluster: bool = False):
    ...
    if cluster and has_person is False and len(faces) > 0:
        ...
        # Threshold 0.7 tuned for normalized 512-d euclidean space; same-person distances
        # typically fall in the 0.3–0.7 range; 1.0 would merge visually distinct faces.
        ...
        for label, data in clustered_data.items():
            ...
            if len(indices) <= 1:
                ...
                data["confidence"] = max(0.0, 1.0 - (diameter / (threshold * CONFIDENCE_SAFETY_MARGIN)))
...
async def prune_duplicates(request: PruneRequest):
    ...
    for pid in request.photo_ids:
        ...
        try:
            ...
            # Commit DB delete first — if this fails we don't touch disk
            c.execute("DELETE FROM faces WHERE photo_id = ?", (pid,))
            c.execute("DELETE FROM photos WHERE id = ?", (pid,))
            conn.commit()
        except Exception as e:
            conn.rollback()
            errors.append(f"DB error for photo {pid}: {e}")
            continue
        finally:
            conn.close()

        # Only delete files after the DB commit succeeded
        try:
            ...
            errors.append(f"File delete warning for {file_path}: {e}")
